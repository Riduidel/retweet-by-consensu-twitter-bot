...
This is an open-source project, done on best effort.

Approved technology lists and technology constraints::
Application will be deployed at minimal effort, so no Kubernetes madness this time.

Target deployment platform::
We expect application to run on CleverCloud systems.

Existing systems and integration standards::
Application is a twitter bot, that must handle twitter API
...
Messages will be formatted according to Twitter constraints.

Size of the software development team::
See https://github.com/Riduidel/retweet-by-consensus-twitter-bot/graphs/contributors[contributors on GitHub] ...

Skill profile of the software development team::
Members of the dev team are supposed to be
* fluent in Java development
* know Quarkus and CDI
* know Twitter API and data model

Nature of the software being built (e.g. tactical or strategic)::
Tactical system built to allow easy technical marketing action
